...
Loop:
    ...
    /*         //now = time(NULL); */
    /*         if(downloadSize - preFileSize >=fileSizepi)/1* if(now - begin >= 1) *1/ */
    /*         { */
    ...
    char *p = mmap(NULL, fileSize, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);//映射
    ret = recvPlus(socketFd, p, fileSize); //接收
    if(-1 == ret)
    {
        sleep(3);
        goto Loop;
