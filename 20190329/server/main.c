...
int main(int argc, char *argv[])
{
    ...
    pthread_t pthreadChild, pthreadTimer;
    threadNode tNode;
    ...
    for(i=0;i<15;++i)
    {
        tNode.ouroboros[i].fdMax=10;
        tNode.ouroboros[i].fd=(int*)calloc(10, sizeof(int));
    }
    pipe(tNode.fds);
    pthread_create(&pthreadChild, NULL, threadFuncChild, &tNode);//启动子线程
    pthread_create(&pthreadTimer, NULL, threadFuncTimer, &tNode);
    while(1)
    {
